...
<p align="center">
  ...
  <span>面向 REDMI K80 Ultra（代号 dali，K80U）的全局、白名单与后台音乐触感模块</span>
</p>
...
A2HHook 是一个面向 **REDMI K80 Ultra（代号 dali，K80U）** 的 Root 音乐触感模块。它在系统音频 HAL 层识别音乐、视频、游戏和系统短音的生命周期，根据全局模式或 10 槽白名单决定何时启用 A2H 音乐触感。
...
>
> [!WARNING]
> 本项目不是通用 Android 音频增强器。当前公开目标是 REDMI K80 Ultra（代号 dali，K80U）；未列出的设备、深度修改过的 ROM、修改过的音频 HAL，以及与其他音频注入模块叠加的场景，都不能视为已验证兼容。
...
| 项目 | 说明 |
| :--- | :--- |
| 目标设备 | REDMI K80 Ultra（代号 dali，K80U） |
| 系统范围 | 运行时按 ELF/语义定位；公开验证覆盖 dali / K80U 的 HyperOS 4.0.0.7 Beta 与 HyperOS 3.0.302 |
...
| 适配结论 | dali / K80U 的 HyperOS 3.0.302 已完成公开回归；其他 ROM 需在目标设备上完成门禁验证 |
...
- 适配 dali / K80U HyperOS 4.0.0.7 Beta 的运行时 inline path，使用 ELF 符号和语义布局定位，不依赖固定 ROM profile。
