...
- 两枚控制中心磁贴：“A2H 全局音乐触感”切换全局/白名单，“游戏时暂停音乐触感”切换游戏时暂停；两者长按均打开同版 WebUI。

当前版本：`v1.5.6-fix`。本版在 v1.5.6 游戏生命周期修复基础上继续处理锁屏解锁、APP 返回时的系统短音误触感与“游戏时暂停”并发应用识别；既有公开版本继续作为独立历史基线保留。

## 下载安装

请到 GitHub Releases 下载：

- `a2h_hook_v1.5.6-fix.zip`
...
6. 需要快捷切换时，将“A2H 全局音乐触感”和“游戏时暂停音乐触感”添加到控制中心；首次使用按 Root 管理器提示授权。点击分别切换全局/白名单与游戏时暂停，长按均打开 WebUI。

“游戏时暂停”默认开启，保持小米对不同应用并存的官方行为。同一应用切换多条扬声器音频流时，两种策略都允许继续音乐触感；关闭该开关后，后台音乐与游戏同时使用扬声器输出也可继续。开关在活动游戏中变化时，模块只在 native 应用与校验成功后建立一次共享 AAudio 静音流，借助事件驱动入口立即调用原厂重算；首次初始化、同值应用、普通包名保存和稳态 watcher 不触发。通话、耳机/蓝牙等非扬声器路由、振动流占用和 HAL 总开关保护不会被绕过。
...
- 输出层始终允许同一应用的多条扬声器流。“游戏时暂停”关闭后遍历完整活跃应用链表，任一应用命中当前全局/10 槽规则即可继续 A2H；开启后由不同应用事件建立两阶段并发 latch：`pending` 会跨过游戏节点短暂消失、正式输出尚未建立的单流过渡，208 字节 RX 页尾 helper 前置补齐 `AudioALSAStreamOut::open()` 成功事件，再遍历 manager 的真实 stream 槽表和每条流的设备向量；至少两条活动扬声器流时提交并持续暂停，已提交状态回落为单流且只剩一个应用时才清除。这样既避免原 HAL 按相同 flags 聚合计数后漏暂停，也不让单应用多流误触发暂停；
...
- K80U / HyperOS 3.0.302 已完成全局/白名单媒体往返、当前音轨/下一首、原厂/放宽策略、FAST/空间音频迁移、配置热更新和稳态回归；后台酷狗概念版进入王者时，策略开启可在第二真实流 open 提交后自动保持 A2H 暂停，音乐会话继续播放且无需暂停/重播，策略关闭则在相同多流过程持续保留 A2H；活动游戏内双向切换策略也已验证一次性事件立即重算。锁屏/解锁无误震和 APP 返回边界继续保留；
