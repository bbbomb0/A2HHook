...
CONFIG_WAKE_FIFO=/data/local/tmp/a2h_config.wake
if [ "${A2H_INOTIFY_CALLBACK:-0}" = "1" ]; then
  ...
  case "$config_event_name" in
    state|game_auto_pause|packages.txt|package_states|config_generation)
      ...
      if [ -p "$CONFIG_WAKE_FIFO" ]; then
        printf 'config\n' > "$CONFIG_WAKE_FIFO" 2>/dev/null || true
      fi
      ;;
  esac
  exit $?
...
AUDIO_WATCHER="$MODDIR/bin/a2h_audio_watch"
...
raw_config_signature() {
  ...
  } | cksum 2>/dev/null) || return 1
  set -- $raw_config_result
  [ "$#" -ge 2 ] || return 1
  printf '%s:%s\n' "$1" "$2"
}
...
prepare_config_wake_fifo() {
  config_wake_ready=0
  rm -f "$CONFIG_WAKE_FIFO" 2>/dev/null || return 1
  mkfifo "$CONFIG_WAKE_FIFO" 2>/dev/null || return 1
  chown 0:0 "$CONFIG_WAKE_FIFO" 2>/dev/null || true
  chmod 0600 "$CONFIG_WAKE_FIFO" 2>/dev/null || {
    rm -f "$CONFIG_WAKE_FIFO" 2>/dev/null
    return 1
  }
  exec 9<> "$CONFIG_WAKE_FIFO" || {
    rm -f "$CONFIG_WAKE_FIFO" 2>/dev/null
    return 1
  }
  config_wake_ready=1
...
wait_for_watch_tick() {
  ...
  if [ "$config_inotify_enabled" = "1" ] &&
     [ "$config_wake_ready" = "1" ] &&
     [ -z "$pending_raw_signature" ] &&
     [ "$config_poll_grace_ticks" -eq 0 ]; then
    watch_wake_event=
    if IFS= read -r -t "$((watch_tick_seconds * watch_health_ticks))" \
      watch_wake_event <&9; then
      watch_elapsed_ticks=1
    else
      watch_elapsed_ticks=$watch_health_ticks
    fi
  else
...
start_audio_watcher() {
  [ -f "$AUDIO_WATCHER" ] || {
    log "audio watcher unavailable reason=missing path=$AUDIO_WATCHER"
    return 1
  }
  chmod 755 "$AUDIO_WATCHER" 2>/dev/null || true
  "$AUDIO_WATCHER" >/dev/null 2>&1 &
  audio_watcher_pid=$!
  if kill -0 "$audio_watcher_pid" 2>/dev/null; then
    log "audio watcher started pid=$audio_watcher_pid"
    return 0
  fi
  audio_watcher_pid=
  log "audio watcher start FAIL"
  return 1
}

stop_audio_watcher() {
  if [ -n "$audio_watcher_pid" ]; then
    kill "$audio_watcher_pid" 2>/dev/null || true
    wait "$audio_watcher_pid" 2>/dev/null || true
  fi
  audio_watcher_pid=
}
...
find_hal_pid() {
  service_pids=$(pidof android.hardware.audio.service-aidl.mediatek 2>/dev/null)
  service_pid=${service_pids%% *}
  ...
  [ -n "$service_pid" ] && { printf '%s\n' "$service_pid"; return 0; }
  service_pids=$(pidof android.hardware.audio.service 2>/dev/null)
  service_pid=${service_pids%% *}
  [ -n "$service_pid" ] && { printf '%s\n' "$service_pid"; return 0; }
...
rm -f "$CONFIG_EVENT_MARKER" 2>/dev/null
start_audio_watcher || true
if prepare_config_wake_fifo; then
  log "watcher config wake=fifo path=$CONFIG_WAKE_FIFO"
else
  log "watcher config wake=fixed-tick fallback=${watch_tick_seconds:-2}s"
fi
...
while true; do
  ...
  if [ "$health_due" = "1" ]; then
    if [ -n "$audio_watcher_pid" ] &&
       ! kill -0 "$audio_watcher_pid" 2>/dev/null; then
      log "audio watcher exited pid=$audio_watcher_pid"
      audio_watcher_pid=
    fi
    [ -n "$audio_watcher_pid" ] || start_audio_watcher || true
  fi
  if [ "$config_inotify_enabled" = "1" ] &&
  ...
  applied_snapshot=
  applied_revision=
  [ ! -r "$CFG_REVISION" ] || IFS= read -r current_revision < "$CFG_REVISION" || true
  [ ! -r "$APPLIED_SNAPSHOT" ] || IFS= read -r applied_snapshot < "$APPLIED_SNAPSHOT" || true
  [ ! -r "$APPLIED_REVISION" ] || IFS= read -r applied_revision < "$APPLIED_REVISION" || true
  need_apply=0
  apply_reason=
